      ...
      <div className="w-full relative z-10">
        ...
        <header className="mb-12">
          ...
          <h1 className="text-4xl md:text-5xl font-black tracking-tighter">
            Manage <span className="text-amber-500 italic">Routes</span>
          </h1>
          <p className="text-zinc-500 mt-2 max-w-md">
            Define new transit corridors and monitor existing operational routes.
          </p>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 items-start mb-16">
          ...
          <div className="lg:col-span-3 space-y-8">
            <SpotlightCard className="p-8 border-white/5">
              <form onSubmit={handleSubmit} className="space-y-8">
                ...
                {success && <p className="text-sm font-medium text-amber-500 bg-amber-500/10 p-4 rounded-xl border border-amber-500/20">Route initialized securely in database.</p>}
                ...
                <div className="space-y-4">
                  ...
                  <input
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 focus:outline-none focus:ring-2 focus:ring-amber-500/50 transition-all text-sm font-medium"
                  />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    ...
                    <input
                      ...
                      placeholder="0.00"
                      className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 focus:outline-none focus:ring-2 focus:ring-amber-500/50 transition-all text-sm font-mono"
                    />
                  </div>
                  <div className="space-y-4">
                    <label className="text-xs text-zinc-500 uppercase font-black tracking-widest flex items-center gap-2">
          ...
          {/* Right Side: Route Schematic Preview */}
              ...
              <div className="relative group overflow-visible">
                ...
                <div className="relative bg-zinc-900 rounded-[2.5rem] border border-white/10 shadow-2xl overflow-hidden scale-95 origin-top">
                  <div className="p-8 border-b border-zinc-800/80 bg-gradient-to-br from-zinc-800/50 to-transparent">
                    ...
                    <h4 className="text-3xl font-black tracking-tight leading-tight min-h-[4rem] text-white break-words">
                  ...
                  <div className="p-8 bg-zinc-900/50">
                    <div className="grid grid-cols-2 gap-8">
                      <div>
                        <p className="text-[10px] text-zinc-500 font-black uppercase tracking-widest mb-1">Daily Access</p>
                        <p className="text-2xl font-black text-white">₹{dailyPrice || "0.00"}</p>
                      </div>
                      <div>
                        <p className="text-[10px] text-zinc-500 font-black uppercase tracking-widest mb-1">Weekly Bundle</p>
                        <p className="text-2xl font-black text-amber-400">₹{weeklyPrice || "0.00"}</p>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        {/* Existing Routes List Section */}
        <section className="space-y-6">
          <div className="flex items-center gap-4">
            <h2 className="text-2xl font-black tracking-tight">Active Routes</h2>
            <div className="h-px flex-grow bg-white/5" />
            <span className="text-zinc-600 text-[10px] font-black uppercase tracking-widest">{routes.length} Total</span>
          </div>

          {routes.length === 0 ? (
            <div className="text-center py-12 text-zinc-500 border border-white/5 rounded-3xl bg-zinc-900/50">
              No routes in database yet. Initialize one above.
            </div>
          ) : (
                    ...
                    <SpotlightCard className="group p-6">
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div className="flex items-center gap-6">
                          <div className="w-12 h-12 rounded-2xl flex-shrink-0 bg-zinc-800 flex items-center justify-center border border-white/5 group-hover:bg-amber-500/10 transition-colors">
                          ...
                          <div>
                            <h4 className="font-bold text-white group-hover:text-amber-400 transition-colors">{route.name}</h4>
                            <div className="flex items-center gap-3 mt-1">
                              ...
                              <span className="text-xs text-zinc-600 font-mono">ID: {route.id}</span>
                            </div>
                          </div>
                        </div>

                        <div className="flex items-center gap-12 border-t md:border-t-0 border-white/5 pt-4 md:pt-0">
                          <div className="text-right">
                            <p className="text-[10px] text-zinc-600 font-black uppercase tracking-widest mb-1">Daily</p>
                            <p className="font-bold text-white">₹{route.daily_price}</p>
                          </div>
                          <div className="text-right">
                            <p className="text-[10px] text-zinc-600 font-black uppercase tracking-widest mb-1">Weekly</p>
                            <p className="font-bold text-amber-400">₹{route.weekly_price}</p>
                          </div>
                        </div>
                      </div>
                    </SpotlightCard>
        ...
        <footer className="mt-20 py-10 border-t border-white/5 text-center">
          <p className="text-zinc-600 text-[10px] font-black uppercase tracking-[0.3em]">
            BusNova Core Systems • Route Management Module
          </p>
        </footer>
      </div>
